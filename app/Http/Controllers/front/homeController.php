...
class homeController extends Controller
{
    ...
    public  function language(Request $request)
    {
		...
		// Session::set('enquiry_mail', 'yes');
		 
		 return Redirect::to('/');

    }
    public function getHotelsHome(Request $request)
    {
        ...
        $data['email']='aranto2006@gmail.com';
	...
	//	 Mail::to('info@hotelsvenues.com')->send(new EnquiryEmail($data));
     	// Mail::to('robingnanaraj1986@gmail.com')->send(new EnquiryEmail($data));
	
		 Mail::to('hotelsvenuessuppilers@gmail.com')->send(new EnquiryEmail($data));
		 
		 	session()->put('enquiry_mail', 'yes');
...
// 		$data['price']=$price;
		...
		*/
		...
		->select('cities.name as city_name', 'hotels.name as hname'
							 ,'hotels.location' ,'hotels.grade', 'hotels.id as id', 'hotels.featured_image','hotels.description')
		->leftjoin('hotels_sub_entity', 'hotels_sub_entity.hotel_id',  '=', 'hotels.id')
		->leftjoin('cities', 'hotels.city', '=', 'cities.id')->where('hotels.status', 'ACTIVE');
		//return $query;
	    ...
	    if( @$data['VenuesDetails']->facilities !== "")
      {
           ...
           // $facilities =  $array;
            ...
            if ($start_date_check >= $end_date_check) 
            {
               ...
               Session::flash('message', 'Start Date greather than End Date, Please select another  Dates'); 
              return redirect('/venuedetails/'.$request->vid)->with('alert-danger', 'Start Date greather than End Date, Please select another  Dates');
             }
        ...
        /*     
           ...
           $sql="select * from bookings where venue_id = '".$request->vid."' and ( start_time between '".$start_date."' and '".$end_date."' 
            OR end_time between '".$start_date."' and '".$end_date."')";
            
            $booking_check = DB::select($sql);
			...
			$request->session()->put('event_title', $request->event_title);					
				  Session::flash('message', 'Selected Dates already Booked, Please select another  Dates'); 

		
		          return redirect('/venuedetails/'.$request->vid)->with('alert-danger', 'Selected Dates already Booked, Please select another  Dates');
			}
	...
	public function Savebookvenues(Request $request)
	{
		...
		*/
		...
		$data['type']='hotel';
		 ...
		 Mail::to('info@hotelsvenues.com')->send(new BookingEmail($data));
// 		 Mail::to('robingnanaraj1986@gmail.com')->send(new BookingEmail($data));
		 ...
		 Mail::to('robingnanaraj1986@gmail.com')->send(new BookingEmail($data));
		  ...
		  Mail::to('hotelsvenuessuppilers@gmail.com')->send(new BookingEmail($data));
...
//        dd($data);
	...
	public function closehomepopup(Request $request)
	{
	    
	   
	  //	$popup=	$request->session()->put('popup', $request->name);
	  	
	  	 $popup = session()->put('popup','yes');
	  	
	  	echo session()->get('popup');
	  	exit;
	}
...
// 		return view('front/suppliers/supplierList', $data);		
// 	}
