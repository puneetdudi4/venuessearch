	  	    ...
	  	    if( @$data['VenuesDetails']->event_type !== "")
      {
       ...
       // $data['User'] = User::find(Auth::guard('web')->user()->id);
		...
		// summary location  city_name  banner_img  featured_image
		 ...
		 $date['start_date']= Session::get('start_date');
		 
		 $date['start_time']= Session::get('start_time');
		 
		 $date['end_date']= Session::get('end_date');
		 
		 $date['end_time']= Session::get('end_time');
		 
		 $date['no_guests']= Session::get('no_guests');
		 
		 $date['vid']= Session::get('vid');
		 $date['hid']= Session::get('hid');
