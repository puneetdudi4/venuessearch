...
class EventTypeController extends Controller
{
    ...
    public function addEventType(Request $request){
        ...
        if(isset($request['image'])){
			...
			$request['image']->move(public_path('storage/eventTypeImg/'),$filename);
		...
		if(isset($request['indoor_image'])){
			...
			$request['indoor_image']->move(public_path('storage/eventTypeImg/'),$indoor_image);
		...
		if(isset($request['outdoor_image'])){
			...
			$request['outdoor_image']->move(public_path('storage/eventTypeImg/'),$outdoor_image);
    ...
    public function updateEventType(Request $request){
        ...
        if(!empty($request['image'])){
            ...
            if($eventType->image != NULL){
                @unlink(public_path('storage/eventTypeImg/'.$eventType->image));
            }
        }else{
		...
		if(!empty($request['indoor_image'])){
            ...
            $request['indoor_image']->move(public_path('storage/eventTypeImg/'),$filename1);
            if($eventType->indoor_image != NULL){
                @unlink(public_path('storage/eventTypeImg/'.$eventType->indoor_image));
            }
        }else{
            $filename1 = $eventType->indoor_image;
        }
		if(!empty($request['outdoor_image'])){
            $filename2 = "Outdoor_".time().'.'.$request['outdoor_image']->getClientOriginalExtension();
            $request['outdoor_image']->move(public_path('storage/eventTypeImg/'),$filename2);
            if($eventType->outdoor_image != NULL){
                @unlink(public_path('storage/eventTypeImg/'.$eventType->outdoor_image));
            }
        }else{
