...
class HotelController extends Controller
{
    ...
    public function addHotelData(Request $request)
    {
		...
		$featured_image = "hotel_main_".time() . '.' . $fimage->getClientOriginalExtension();
					
		$fimage->move(public_path('uploads/hotels_featured_images/'),$featured_image);
        }
        else
        {
		...
		*/
        ...
        $this->validate(request(), [
            ...
            //'featured_image' => 'required|featured_image|mimes:png,jpeg,jpg',
        ...
        if ($ispresent) {
                ...
                $filename = "licence_".time() . '.' . $request->licence->getClientOriginalExtension();
				
			
               // $request->licence->move(storage_path('app/public/merchantUser/'), $filename);
				
				$request->licence->move(public_path('uploads/licence/'),$filename);
	
		
         } else {
                $filename  = @$ispresent->licence;
         }
    ...
    public function submitMerchantIncludeHotel(Request $request)
    {
	   ...
	   if($request->hasfile('fimage'))
		...
		$featured_image = "venue_main_".time() . '.' . $fimage->getClientOriginalExtension();
	   ...
	   if($request->hasfile('specification'))
        {
        //$featured_image = DB::table('draft_images')->select('image_name')->where('image_type','featured_image')->value('image_name');
		$specification=$request->file('specification');

		$specification_pdf = "venue_specification_".time() . '.' . $specification->getClientOriginalExtension();
					
		$specification->move(public_path('uploads/specification/'),$specification_pdf);
        }
        else
        {
    ...
    public function getHotelMerchantEditInclude($id)
    {
        ...
        $event = explode(",", $data['hotelSubEntity']->event_type);
		 ...
		 $facilities = explode(",", $data['hotelSubEntity']->facilities);
        ...
        //$data['gallery_images'] = Hotel_image_gallery::getImagesByUsingHotelId($id);
		 ...
		 $data['packages'] = Hotel_package_venue::getPackagesByUsingHotelVenueId($id);
    ...
    public function updateHotelsIncludeMerchant(Request $request)
    {
	   ...
	   $featured_image ='';
        ...
        $this->validate(request(), [
		 ...
		 if($packages=$request->file('packages_file'))
			   {
				 ...
				 // print_r(count($packages));
				  ...
				  $pcount=count($packages);
				  // $i=1;
                for($pack=0; $pack<$pcount; $pack++)
				 {
					 
					 $packages_file=$packages[$pack];
					
					 
					  $package_name=$packages_name[$pack];
					 
                    $name=$packages_file->getClientOriginalName();
					
					 $package_file = "packages_".$pack."_".time() . '.' . $packages_file->getClientOriginalExtension();
					
					//public/uploads/hotels_gallery_images
					
                    $packages_file->move(public_path('uploads/packages/'),$package_file);
                   // $gallery_images[]=$gallery_name;
					
					
					
					
					$hotel_image= Hotel_package_venue::create([
                    'hotel_id' => $request->hotel_id ,
                    'hotel_sub_entity_id' => $id,
                    'order' => 0,
                    'name' => $package_name,
                    'pdf' => $package_file,
                    'is_deleted' => 0
        ...
        $hotelInclude['type'] = $request->type;
         ...
         $hotelInclude['cost'] = $request->cost;
          
         $hotelInclude['discount'] = $request->discount;
         
         $hotelInclude['commission'] = $request->commission;
         
         $hotelInclude['final_price'] = $request->final_price;
		...
		$hotelInclude['specification_pdf'] = $specification_pdf;
    ...
    public function hotelMerchantIncludeView($id)
    {
     ...
     public function getmenucategory(Request $request)
	 {
		...
		{
			?>
			<option value="<?php echo $menu_cat->id?>"><?php echo $menu_cat->title?></option>
			<?php
		}			
	 }
...
//        dd($capData , $request);
        ...
        if (count($request['title']) == count($request['cost'])) {
			
			
            for ($i = 0; $i < count($request['title']); $i++) {
				
				if($request['title'][$i] !=''  && $request['cost'][$i] !='')
				{
				
                if (isset($request['image'][$i])) {
                    $imageMenu = "menu_".time() . '.' . $request['image'][$i]->getClientOriginalExtension();
					
                    $request['image'][$i]->move(public_path('storage/hotels_sub_entity_menu/'), $imageMenu);
                } else {
                    $imageMenu = NULL;
                }

                if (isset($request['doc_file'][$i])) {
                    $document = "menu_".time() . '.' . $request['doc_file'][$i]->getClientOriginalExtension();
                    $request['doc_file'][$i]->move(public_path('storage/hotels_sub_entity_menu/'), $document);
                } else {
                    $document = NULL;
                }
                $capData[] = [
...
//            dd($value);
    ...
    public function submitFormHotelsIncludeMenu(Request $request)
    {
        ...
        $menuUpdate = IncludeHotelMenu::where('id', $id)->first();;
		
	  
/*            $imageFile = "menu_".time() . '.' . $request['image']->getClientOriginalExtension();
            $request['image']->move(public_path('storage/hotels_sub_entity_menu/'), $imageFile);
            if ($menuUpdate->image != NULL) {
                @unlink(public_path('storage/hotels_sub_entity_menu/' . $menuUpdate->image));
            }
        } else {
			//public_path('storage/hotels_sub_entity_menu/')
            $imageFile = $menuUpdate->image;
        }
		*/

        if (!empty($request['doc_file'])) {
            $doc_file = "menu_".time() . '.' . $request['doc_file']->getClientOriginalExtension();
            $request['doc_file']->move(public_path('storage/hotels_sub_entity_menu/'), $doc_file);
            if ($menuUpdate->doc_file != NULL) {
                @unlink(public_path('storage/hotels_sub_entity_menu/' . $menuUpdate->doc_file));
            }
        } else {
		...
		$menuUpdate['menu_type'] = $request['menu_type'];
		
		$menuUpdate['category_id'] = $request['category_id'];
		
		
		
      //  $menuUpdate['image'] = $imageFile;
        $menuUpdate['doc_file'] = $doc_file;
		
		$menuUpdate['cost'] = $request['cost'];
		
		$menuUpdate['discount'] = $request['discount'];
   ...
   public function cancelationrules(Request $request)
   {
	   ...
	   $data['hotel_id']=$hotelId;
	    ...
	    $data['cancelrule'] = CancelRule::where('hotelid', $hotelId->id)->first();		
			
			
        return view('merchant.HotelMerchant.cancelrules', $data);
   }
   
   
   public function savecancelrules(Request $request)
   {
	   
	
	   $ispresent = CancelRule::where('id', $request->cid)->first();
   ...
   $this->validate(request(), [
		...
		if ($ispresent) {
		     ...
		     CancelRule::where('id', $ispresent->id)->update([
                'hotelid' => $request->hotelid,
                
                'sixmonth' => $request->sixmonth,
                'threemonth' => $request->threemonth,
                'onemonth' => $request->onemonth,
                'fifteenday' => $request->fifteenday,
                'oneweek' => $request->oneweek
				
            ]);
			
			            return redirect('merchant/hotel/cancelationrules');
    ...
    public function editMenu($id, $hotelId) {
       $data['menu']= $menu = IncludeHotelMenu::find($id);
...
//        return $this->hotelMerchantIncludeList($menu);
        return view('merchant.HotelMerchant.edit_menu', $menu, $data);
    }
   ...
   public function booking_ajax_update (Request $request)
   {
	   ...
	   $booking = Booking::find($request->booking_id);
        ...
        $booking->end_time = $request->end_date . ' ' . $request->end_time;
      ...
      public function booking_ajax_delete (Request $request)
   {
    ...
    public function ajaxUpdate(Request $request)
    {
        ...
        $booking->end_time = $end_date . ' ' . $request->finish_time;
        $booking->event_title = $request->event_title;
        $booking->is_booked = $request->is_booked;
        $booking->save();
        $bookings = Booking::where('hotel_id', $hotel->id)->get();
	...
	public function getbookingCalendar(Request $request)
	{
	    ...
	    $booking['end_date']=date('Y-m-d',strtotime($booking->end_time));
	...
	public function getbookinghistory()
	{
	   ...
	   // $data['bookings'] = Booking::where('hotel_id', $hotelId->id)->get();	
        ...
        $data["bookings"] = DB::table('bookings as b')
                                ...
                                ->where('b.hotel_id', $hotelId->id)
                               ->orderBy('b.id', 'desc')
                                ->get();
								
			
		
        return view('merchant.HotelMerchant.bookinghistory', $data);
	}
	...
	public function getbookingdetails($id)
	{
        ...
        return view('merchant.HotelMerchant.bookingdetails', $data);
