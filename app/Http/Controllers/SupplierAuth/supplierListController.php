...
class supplierListController extends Controller
{
    ...
    public function getHotelSupplierView($id)
    {
        ...
        // $gallery_images = DB::table('supplier_image_gallery')->where('supplier_id',$id)->get();
        $data['supplier_id'] = $id;
   ...
   /*	   if($request->hasfile('fimage'))
		...
		*/
		...
		$featured_image = "supllier_main_".time() . '.' . $fimage->getClientOriginalExtension();
                ...
                foreach($galleries as $file)
				 {
                    //$name=$file->getClientOriginalName();
					 ...
					 $gallery_name = "suppliermore_".$i."_".time() . '.' . $file->getClientOriginalExtension();
					
					//public/uploads/hotels_gallery_images
					
                    $file->move(public_path('uploads/hotels_featured_images/'),$gallery_name);
                ...
                foreach($galleries as $file)
				 {
                    //$name=$file->getClientOriginalName();
					 ...
					 $gallery_name = "supplier_".$i."_".time() . '.' . $file->getClientOriginalExtension();
    ...
    public function getSupplierProductView($id)
    {
        ...
        $data['Supplier'] = Supplier::find(Auth::guard('supplier')->user()->id);
		 ...
		 $data['gallery_images'] = DB::table('supplier_image_gallery')
	             ->select('image')
         ...
         if($request->hasfile('fimage'))
             {
               ...
               // foreach($request->file('fimage') as $file)
               // {
                    $featured_image ='main_product_'.time().rand(1,100).'.'.$file->extension();
                    $file->move(public_path('uploads/hotels_featured_images'), $featured_image);  
                 //   $files[] = $name;  
               // }
             }
            ...
            DraftImages::truncate();
						 ...
						 if($galleries=$request->file('gallery'))
			   {
                ...
                foreach($galleries as $file)
				 {
                    //$name=$file->getClientOriginalName();
					 ...
					 $gallery_name = "products_".$i."_".time() . '.' . $file->getClientOriginalExtension();
                ...
                //foreach ($gallery_image[0] as $gi) {
                    SupplierImageGallery::create([
                        'supplier_id' => $supplierId->id,
                        'supplier_product_id' => $productId->id,
                        'order' => 0,
                        'description' => 'none',
                        'image' => $gallery_name,
                        'is_deleted' => 0
            ...
            if($request->hasfile('fimage'))
             {
               ...
               // foreach($request->file('fimage') as $file)
               // {
                    $filename ='main_product_'.time().rand(1,100).'.'.$file->extension();
                    $file->move(public_path('uploads/hotels_featured_images'), $filename);  
                 //   $files[] = $name;  
               // }
             }
             else
                ...
                foreach($galleries as $file)
				 {
                    //$name=$file->getClientOriginalName();
					 ...
					 $gallery_name = "product_".$i."_".time() . '.' . $file->getClientOriginalExtension();
